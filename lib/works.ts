import type { Work } from '@/types/work'

// Data for Works Section
export const featuredWorks: Work[] = [
  {
    slug: 'flatmates',
    title: 'UX Case Study: Flatmates Revised Prototype',
    subtitle: 'RMIT Online presentation work',
    roles: ['UX Research', 'Prototyping'],
    term: 'Mar 2023 - May 2023',
    thumbnail: '/work-thumbnails/flatmates.png',
  },
  {
    slug: 'datavis',
    title: 'MetaStanza: Web component for data visualization ',
    subtitle: 'Showcase of MetaStanza',
    roles: ['UI Design', 'Frontend'],
    term: 'Aug 2020 - Mar 2021',
    thumbnail: '/work-thumbnails/datavis.png',
  },
  {
    slug: 'jess-portfolio',
    title: 'Jess J – Support Coordinator Website',
    subtitle: 'For a freelance Support Coordinator ',
    roles: ['Web Design', 'WIX'],
    term: 'Sep 2023 - Oct 2023',
    thumbnail: '/work-thumbnails/jess-portfolio.png',
  },
  {
    slug: 'coffee-flyer',
    title: 'Flyer for a coffee shop',
    subtitle:
      "Created a flyer design to inform about a local coffee shop's participation in an event",
    roles: ['Flyer Design'],
    term: 'Dec 2022 - Jan 2023',
    thumbnail: '/work-thumbnails/coffee-flyer.png',
  },
  {
    slug: 'seek',
    title: 'UX Case Study: Revised Prototype for the SEEK Mobile App',
    subtitle: 'Created for coursework at Kaplan Business School',
    roles: ['UI Design', 'UX Research', 'Prototyping', 'Figma'],
    term: 'Sep 2024',
    thumbnail: '/work-thumbnails/seek.png',
  },
]
